...
const service = new app_1.AccountsService(new AccountRepository_1.AccountRepository());
afterAll(() => {
    service.reset();
    return;
});
...
test("can make with draw", () => {
    ...
    expect(service.makeWithDraw("withdraw-account", 50)).toThrow("Account does not exist");
    return;
});
// to-be implemented
test("can undo", () => {
